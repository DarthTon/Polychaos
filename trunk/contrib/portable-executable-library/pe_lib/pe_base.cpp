...
//Returns section from RVA
section& pe_base::section_from_name( const std::string& name )
{
    //Search for section
    for (section_list::iterator i = sections_.begin(); i != sections_.end(); ++i)
    {
    ...
    throw pe_exception( "No section found by presented name", pe_exception::no_section_found );
}
...
//Strip section from section list
bool pe_base::strip_section(const section& s)
{
    auto sec = std::find_if(sections_.begin(), sections_.end(), section_ptr_finder(s));
...
//Sets linker version
void pe_base::set_linker_version(uint8_t version)
{
    props_->set_linker_version(version);
}
